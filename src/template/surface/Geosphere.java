...
public abstract class Geosphere extends Surface {
    ...
    public void melt(GeoCell cell, float maxHeight) {
        ...
        if (height > maxHeight) {
            ...
            massToChange = calcMass(diff, Planet.instance().getCellArea(), bottomType);
    ...
    public void formNewRock(GeoCell cell, float maxHeight) {
        ...
        height = calcHeight(eb.getSediments(), Planet.instance().getCellArea(), SEDIMENT);
        if (height > maxHeight) {

            diff = (height - maxHeight);

            massBeingDeposited = calcMass(diff, Planet.instance().getCellArea(), SEDIMENT);
    ...
    public void windErosion(GeoCell spreadFrom) {
        float height = calcHeight(0.1f, Planet.instance().getCellArea(), SEDIMENT);
    ...
    public void convertTopLayer(GeoCell spreadFrom, float height) {
        ...
        if (eb.getSediments() == 0 && !spreadFrom.hasOcean()
            ...
            rockMass = calcMass(height, Planet.instance().getCellArea(), SEDIMENT);
    ...
    public void spread(ArrayList<GeoCell> lowestList, GeoCell spreadFrom) {
        ...
        if (lowestList.size() > 0) {
            ...
            if (eb.getSediments() > 0) {
                ...
                mass = calcMass(diff, Planet.instance().getCellArea(), SEDIMENT);
                mass = calcFlow(mass);
    ...
    public void updateLavaFlows(int x, int y) {
        ...
        if (toUpdate.getMoltenRockFromSurface() > 10) {
            ...
            if (lowest != null && lowest != toUpdate) {
                ...
                float mass = calcMass(diff, Planet.instance().getCellArea(), LAVA);
    ...
    public void updateGeology(int x, int y) {
        ...
        if (Planet.instance().isTimeScale(Geological)) {
            cell.cool(1);
        } else if (!Planet.instance().isTimeScale(None)) {
