 ...
 * @author Richard
 */
public abstract class Conduction extends Task {
  
  protected final SurfaceMap surface;
  ...
  public float[] calculateHeatConductance(int x, int y, float zLength, 
    ...
    Vector2f left = new Vector2f(x + 1, y);//0
    Vector2f right = new Vector2f(x - 1, y);//1
    Vector2f forward = new Vector2f(x, y + 1);//2
    Vector2f back = new Vector2f(x, y - 1);//3
